...
**Next action:** Start of session, recap for Chris before diving in:
1. **Open decision from last session (not yet answered):** `backend/app.py` was lost (container
   recreate wiped uncommitted work) and was regenerated by infra automation
   (`learning-codeserver-bot`), not retyped by her — verified as a genuine infra event via git
   blame, not tampering, but it cuts against the tutor's "she writes every line" rule (§4). Ask
   her to choose before M4 builds on it: (a) a quick 2-3 min verbal walkthrough of the
   reconstructed file, or (b) retype `backend/app.py` herself from scratch as a review exercise.
2. **Vite dev server:** not persistent across sessions — needs `npm run dev` (from `frontend/`)
   restarted each session. URL is `https://code.wakehub.org/absproxy/5173/` (or
   `code.home.wakehub.org`) — note `/absproxy/`, not `/proxy/` (redirect-loop fix from
   2026-08-11). Start it and give her the link before resuming M3 content.
3. **Then continue M3:** tour `main.tsx`/`App.tsx`/`index.html`, connect to the Spring Boot
   "bootstrap" mental model, then start building components, props, state, and a `Task`
   TypeScript interface — no backend calls yet (that's M4). Not started yet — session ended
   right as we were about to open `main.tsx`.
